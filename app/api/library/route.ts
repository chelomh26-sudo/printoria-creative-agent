...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await supabase.from("creative_brand_assets").insert({ name: file.name, storage_path: path, mime_type: file.type, asset_class: assetClass, category }).select("*").single();
    if (error) throw error;
